---SQL Server Build Checklist: post-install configuration for SQL Server
-- best to execute as text to save all output easily for auditing (just copy report text, and paste to checklist at the end)
--store audit scripts in doc, or vault(?) to prove config, afterwards (trimmed down from full audit)
--SELECT GETDATE()
SELECT SYSDATETIME() -- as per Calin O.'s recommendation Nov 27th


--line 2
SELECT @@SERVERNAME
--SERVERPROPERTY()
-- beginning of sys configurations table changes
USE master
go
...
--line 6 Set Memory, after first, and second to last columns are at 100/85%
	SELECT   (si.physical_memory_kb / 1024) as Physical_memory_in_Mbytes,--FROM sys.dm_os_sys_info si
...
CROSS JOIN (
    SELECT [value_in_use] AS [max_server_memory]
    FROM sys.configurations --sc
    WHERE [name] = 'max server memory (MB)') AS c
CROSS JOIN (SELECT [value_in_use] AS [min_server_memory]
    FROM sys.configurations -- will show you what SQL is using now, if you like
    WHERE [name] = 'min server memory (MB)') AS c2;
        -- GB available memory - there should be 1GB at least 
        -- MB used, MB avail for the OS

-- the right value is to take 85% of first column, and ensure max_Server_memory (2nd to last)
-- ensure that max_Server_memory is no higher than 85% of Physical_Memory_In_bytes
EXEC sys.sp_configure N'min server memory (MB)', N'2201'
GO -- we should always set a couple of gigs for sql to start, or 4200
SELECT ((si.physical_memory_kb / 1024)*0.85) as Physical_memory_in_Mbytes FROM sys.dm_os_sys_info si
EXEC sys.sp_configure N'max server memory (MB)', N'6880' -- round up from select above
-- this should be set to around 15% less than total max memory, but lower for VMs - Calin O. warns of over-provisioning, thus 75% is safer
GO
RECONFIGURE WITH OVERRIDE
GO

-- Line 7 validate sql authentication mode
Select SERVERPROPERTY('IsIntegratedSecurityOnly')
GO -- 0 means mixed mode,the often standard, but better if only 1, meaning Active Directory integrated
 
exec xp_loginconfig 'login mode';
-- or more explictly we confirm mixed mode in use, althought AD is preference

USE master
GO

-- check to see if enterprise
SELECT SERVERPROPERTY('Edition')
-- line 8 Backup Compression (ent or dev only)
EXEC sys.sp_configure N'backup compression default', N'1'
GO -- ensure it's used (unless STD, which'll have no effect)
RECONFIGURE WITH OVERRIDE
GO

-- Line 9
-- add temp db create code, conditional on drives and physical cores (24 ht, means 12 temp data files, equal sized, no growth)

-- Line 9 if relocation is required, fix temp db location:
...
/*
--Physically move these files to the new location T:\MSSQL\Data\
--Often the std temp location specified for the tempdb files is T:\MSSQL\Data\ 
--If you move the files to different location, modify the example to point to that location.
--
--Change the location of each file by using ALTER DATABASE.  recently folder naming has caused confusion, 
--so if the folders are out of whack from master files query below, use this code below to move them
--Copy Code 

USE master;
GO
ALTER DATABASE tempdb 
MODIFY FILE (NAME = tempdev1, FILENAME = 'T:\MSSQL\Data\tempdb.mdf');
GO
ALTER DATABASE tempdb 
MODIFY FILE (NAME = tempdev2, FILENAME = 't:\MSSQL\Data\tempdb2.ndf');
GO
ALTER DATABASE tempdb 
MODIFY FILE (NAME = tempdev3, FILENAME = 't:\MSSQL\Data\tempdb3.ndf');
GO
ALTER DATABASE tempdb 
MODIFY FILE (NAME = tempdev4, FILENAME ='t:\MSSQL\Data\tempdb4.ndf');
GO

ALTER DATABASE  tempdb 
MODIFY FILE (NAME = templog, FILENAME = 't:\MSSQL\Data\templog.ldf');
GO
-- end fix temp db location fix
*/

-- to validate how temp db is setup after you've adjusted it
SELECT name, physical_name
FROM sys.master_files
WHERE database_id = DB_ID('tempdb');
GO
/*at the very least, set the file size growth well, if only two files
USE [master]
GO
ALTER DATABASE [tempdb] MODIFY FILE ( NAME = N'tempdev', FILEGROWTH = 524288KB )
GO
ALTER DATABASE [tempdb] MODIFY FILE ( NAME = N'templog', FILEGROWTH = 262144KB )
GO

*/
--EXEC sys.xp_instance_regread N'HKEY_LOCAL_MACHINE', N'Software\Microsoft\MSSQLServer\MSSQLServer', N'DefaultData'

-- Line 10, DB Locations (select out where they are currently on the server, add code here)
USE [master]
GO -- confirm f:\mssql\data\ (if not already done - designate the drives here)
--EXEC xp_instance_regwrite N'HKEY_LOCAL_MACHINE', N'Software\Microsoft\MSSQLServer\MSSQLServer', N'DefaultData', REG_SZ, N'F:\MSSQL\Data\'
GO-- logs to l
--EXEC xp_instance_regwrite N'HKEY_LOCAL_MACHINE', N'Software\Microsoft\MSSQLServer\MSSQLServer', N'DefaultLog', REG_SZ, N'L:\MSSQL\Data\'
GO -- backups to f
--EXEC xp_instance_regwrite N'HKEY_LOCAL_MACHINE', N'Software\Microsoft\MSSQLServer\MSSQLServer', N'BackupDirectory', REG_SZ, N'F:\MSSQL\Backups\'
GO



-- Line 11 Maximum Degree of Parallelism  MAXDOP
EXEC sys.sp_configure N'max degree of parallelism', N'8'
--EXEC sys.sp_configure N'max degree of parallelism', N'4' -- for lower cpu servers in azure
GO -- we start with 8, but that doesn't mean users cannot use Maxdop=# cores available to SQL
RECONFIGURE WITH OVERRIDE
GO
-- select * from sys.configurations  -to validate where we are

-- Line 12
GO -- Often the standard is to leverage the common language runtime integration feature
EXEC sys.sp_configure 'clr enabled', 1;  
RECONFIGURE WITH OVERRIDE
GO

-- Line 13, remove drive and log file indexing, under drive properties, first window (windows setting)

-- Line 14 Optimise for Adhoc Procedures. (not for a BI team)

EXEC sys.sp_configure N'optimize for ad hoc workloads', N'1'
GO
-- Line 15 Index default fill factor
EXEC sys.sp_configure N'fill factor (%)', N'95'
GO

-- Line 16 Cost Threshold for Degree of Parallelism
EXEC sys.sp_configure N'cost threshold for parallelism', N'15'
GO  -- how about 2 seconds, why would want things to run this slow...

-- Line 17 Blocked Process Threshold
EXEC sys.sp_configure N'blocked process threshold (s)', N'5'
GO

--- check if 1, otherwise update
--populated temp with name, minimum, maximum, config_value, run_value
-- make sure run_value=config_value
--IF (EXECUTE sp_configure 'xp_cmdshell');
-- if not 
go
reconfigure
go
--exec sp_configure 'xp_cmdshell', 1  -- avoid as much as possible
GO

-- last of the general config settings
GO  
RECONFIGURE;  
GO  
...
-- Line 19 - Set SA password, only if enabled, which is a non-default setup
USE master
GO
--ALTER LOGIN sa SET PASSWORD ='SomethingKnobodyCanGetAndLongwith@#$$!WADSF'
GO

-- Line 20 Policy Management (script out from prod?)


--- Line 22 - Database Mail Setup: enable, set profiles, operators...
------------------------------------------------------------
---------------------------------------- Database Mail Setup
------------------------------------------------------------
...
if not exists (select *
                ...
                where name = 'Support_dba')
  exec msdb.dbo.sysmail_add_profile_sp @profile_name = N'Support_dba',
                                       @description  = N'Database Mail profile enabled on MS SQL servers Managed by Alithya'


if not exists (select *
                 from dbo.sysmail_account
                where name = 'Primary' OR  name = 'Support_dba')
				
  exec msdb.dbo.sysmail_add_account_sp @account_name            = 'Support_dba',
                                       @email_address           = 'Support_dba@alithya.com',
                                       @display_name            = @@servername,
                                       @replyto_address         = 'Support_dba@alithya.com',
                                       @description             = 'Administrative emails for DBAs',
                                       @mailserver_name         = 'smtp.alithya.com',
									   -- if in AZURE  @mailserver_name         = 'transact2.silverpop.com',smarthost.elm.shoprite.com
...
if not exists (select *
                 from dbo.sysmail_account        A inner join
                      dbo.sysmail_profileaccount P on
                                                 P.account_id = A.account_id
                where A.name = 'Support_dba')
  exec msdb.dbo.sysmail_add_profileaccount_sp @profile_name    = N'Support_dba',
                                              @account_name    = N'Support_dba',
                                              @sequence_number = 1

USE msdb  -- when adding failover for 1st time, start here, but section by section
go
if not exists (select *
                 from dbo.sysmail_account
                where name = 'Failover')
  exec msdb.dbo.sysmail_add_account_sp @account_name            = 'Failover',
                                       @email_address           = 'Support_dba@alithya.com',
                                       @display_name            = @@servername,
                                       @replyto_address         = 'Support_dba@alithya.com',
                                       @description             = 'Administrative emails for DBAs -Failover SMTP server',
	                                   @mailserver_name         = 'smtp2.alithya.com',
                                       @mailserver_type         = 'SMTP',
                                       @port                    = 25,
                                       @username                = NULL,
                                       @password                = NULL,
...
if not exists (select *
                 from dbo.sysmail_account        A inner join
                      dbo.sysmail_profileaccount P on
                                                 P.account_id = A.account_id
                where A.name = 'Failover') --'DBA Profile'
  exec msdb.dbo.sysmail_add_profileaccount_sp @profile_name    = N'Support_dba', -
                                              @account_name    = N'Failover',
                                              @sequence_number = 2
if not exists (select *
                 from msdb.dbo.sysmail_account A inner JOIN 
				 msdb.dbo.sysmail_profileaccount P on P.account_id = A.account_id
                where A.name LIKE '%dba%')
				 exec msdb.dbo.sysmail_add_profileaccount_sp @profile_name    = N'Support_dba', -- this may vary!!!
                                              @account_name    = N'Support_dba',
                                              @sequence_number = 1 -- check in gui afterwards too
...
-- stop here if only just adding failover
if not exists (select *
                 from dbo.sysmail_principalprofile L inner join
                      dbo.sysmail_profile          P on
                                                   P.profile_id = L.profile_id
                where P.name like '%DBA%') --AND l.is_default=0)
  execute msdb.dbo.sysmail_add_principalprofile_sp @principal_name = 'public',
                                                   @profile_name   = 'Support_dba',  --DBA Profile'
                                                   @is_default     = 1 ;
GO
-- mail test below after operator created

-- Line 25, create DBA operator
USE [msdb]
GO
EXEC msdb.dbo.sp_add_operator @name=N'Support_dba', 
		@enabled=1, 
		...
		@pager_days=127, 
		@email_address=N'Support_dba@alithya.com'
GO
-- test notification to operator
sp_notify_operator @profile_name=DEFAULT,@id=DEFAULT,
 @name='Support_dba', @subject='Support_dba test message',@body='test alert from new server build by Alithya'

 -- validate if any mail sent recently
 SELECT COUNT(mailitem_id) RecentSPapm FROM msdb.dbo.sysmail_mailitems
WHERE send_request_date LIKE '%2018%'

-- Line 23, 24, Agent setup, history, 22 again to confirm fail safe operators.
 -- set SQL agent to use the dbmail profile created in previous line steps  (added by Roman R. 1 nov, 2017)
 USE [msdb]
GO
EXEC msdb.dbo.sp_set_sqlagent_properties @email_save_in_sent_folder=1
GO
EXEC master.dbo.xp_instance_regwrite N'HKEY_LOCAL_MACHINE', N'SOFTWARE\Microsoft\MSSQLServer\SQLServerAgent', N'DatabaseMailProfile', N'REG_SZ', N'Support_dba'
GO

-- set idle condition to avoid noise in error log, every startup
USE [msdb]
GO
EXEC msdb.dbo.sp_set_sqlagent_properties @cpu_poller_enabled=1, 
		@idle_cpu_duration=3600  -- cpu must be idle for 30 minutes below 10%
GO
USE [msdb]

-- check to see if enabled yet:
declare @rc VARCHAR(MAX),
@failsafeoperator varchar(25)

exec @rc = master.sys.xp_instance_regread

'HKEY_LOCAL_MACHINE','SOFTWARE\Microsoft\MSSQLServer\SQLServerAgent','AlertFailSafeOperator', @failsafeoperator output
 -- just like line 22 continued sql agent setup
select @failsafeoperator
IF @failsafeoperator IS NULL (EXEC master.dbo.sp_MSsetalertinfo @failsafeoperator=N'Support_dba', @notificationmethod=1
								EXEC msdb.dbo.sp_set_sqlagent_properties @email_save_in_sent_folder=1)
GO
USE [msdb]
GO
EXEC msdb.dbo.sp_set_sqlagent_properties @email_save_in_sent_folder=1, 
		...
		@idle_cpu_duration=3600, 
		@databasemail_profile=N'Support_dba', 
		@use_databasemail=1

		-- end of any dbmail related configuration
GO
USE [msdb]
GO
...
EXEC msdb.dbo.sp_syspolicy_add_condition @name=N'In Full Recovery Mode', @description=N'', @facet=N'Database', @expression=N'<Operator>
  <TypeClass>Bool</TypeClass>
  <OpType>AND</OpType>
  <Count>2</Count>
  <Operator>
...
Declare @condition_id int
EXEC msdb.dbo.sp_syspolicy_add_condition @name=N'Last Log Backups', @description=N'', @facet=N'IDatabaseMaintenanceFacet', @expression=N'<Operator>
  <TypeClass>Bool</TypeClass>
  <OpType>GE</OpType>
  <Count>2</Count>
  <Attribute>
...
Declare @condition_id int
EXEC msdb.dbo.sp_syspolicy_add_condition @name=N'T-Log VLF Check Condition', @description=N'If a database has more than 500 VLF''s we will be notified.  Too many VLF slow down the restore process and cause slow failovers.', @facet=N'Database', @expression=N'<Operator>
  <TypeClass>Bool</TypeClass>
  <OpType>LE</OpType>
  <Count>2</Count>
  <Function>
...
EXEC msdb.dbo.sp_syspolicy_add_object_set @object_set_name=N'Check Backup Completion_ObjectSet', @facet=N'IDatabaseMaintenanceFacet', @object_set_id=@object_set_id OUTPUT
Select @object_set_id

Declare @target_set_id int
EXEC msdb.dbo.sp_syspolicy_add_target_set @object_set_name=N'Check Backup Completion_ObjectSet', @type_skeleton=N'Server/Database', @type=N'DATABASE', @enabled=True, @target_set_id=@target_set_id OUTPUT
...
Declare @object_set_id int
EXEC msdb.dbo.sp_syspolicy_add_object_set @object_set_name=N'Recent Log Backups_ObjectSet_1', @facet=N'IDatabaseMaintenanceFacet', @object_set_id=@object_set_id OUTPUT
Select @object_set_id

Declare @target_set_id int
EXEC msdb.dbo.sp_syspolicy_add_target_set @object_set_name=N'Recent Log Backups_ObjectSet_1', @type_skeleton=N'Server/Database', @type=N'DATABASE', @enabled=True, @target_set_id=@target_set_id OUTPUT
Select @target_set_id

EXEC msdb.dbo.sp_syspolicy_add_target_set_level @target_set_id=@target_set_id, @type_skeleton=N'Server/Database', @level_name=N'Database', @condition_name=N'In Full Recovery Mode', @target_set_level_id=0
...
------------------------------------------------------------
-- Implement file growth standards to the Model database
----- fix bad user database file growth issues before that arrive
USE [master]
GO
ALTER DATABASE [model] MODIFY FILE ( NAME = N'modeldev', FILEGROWTH = 256MB )
GO
ALTER DATABASE [model] MODIFY FILE ( NAME = N'modellog', FILEGROWTH = 512MB )
GO
-- plus Calin's suggestion:
GO
ALTER DATABASE [model] SET RECOVERY SIMPLE WITH NO_WAIT  -- risk mitigation for full log files
GO


-- from Calin's Setup Doc, 
--  https://docs.google.com/document/d/1TkdGcEbyPE4TD52XJPmai6UrJqGV9qwY-DhlxBWS4Zs/edit#
---what's missing above (and in draught also here  https://drive.google.com/open?id=0B2ZhPmp6TEBxQ3lBNG8tZzdVT00 )
-- fix svc account to have locked pages in memory grant


-- enable remote dAC is does that count down on us for security?
--- enable Asynchronous stats in high-volume dbs?  RR? I'd love to see how that'd work for SR

--- go through which traces of 1118,7 - 4199, 3226 to apply per server
DBCC TRACEON (4199)

--  Line 26? linked servers
/*
USE [master]
GO
EXEC master.dbo.sp_addlinkedserver @server = N'InternalServerNameFQDN', @srvproduct=N'SQL Server'
EXEC master.dbo.sp_addlinkedsrvlogin @rmtsrvname=N'InternalServerNameFQDN',@useself=N'True',@locallogin=NULL,@rmtuser=NULL,@rmtpassword=NULL
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'collation compatible', @optvalue=N'false'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'data access', @optvalue=N'true'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'dist', @optvalue=N'false'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'pub', @optvalue=N'false'EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'rpc', @optvalue=N'true'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'rpc out', @optvalue=N'true'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'sub', @optvalue=N'false'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'connect timeout', @optvalue=N'0'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'collation name', @optvalue=null
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'lazy schema validation', @optvalue=N'false'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'query timeout', @optvalue=N'0'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'use remote collation', @optvalue=N'true'
EXEC master.dbo.sp_serveroption @server=N'InternalServerNameFQDN', @optname=N'remote proc transaction promotion', @optvalue=N'true'
GO

*/


-- more custom dba procs for utility to adjust?  or to drop from this point on 
-- working on an updated stats query that'll leave any active objects alone	
------------------------------------------------------------
---------------------------------------------- User messages
------------------------------------------------------------
...
if (not exists (select * from master.dbo.sysmessages where error = 50001))
   exec  master.dbo.sp_addmessage @msgnum = 50001, @severity = 10, 
            @msgtext = N'proc_blocked has detected a blocking condition longer than %d ms', 
            @lang = 'us_english', @with_log ='true' 
go
if (not exists (select * from master.dbo.sysmessages where error = 50002))
   exec  master.dbo.sp_addmessage @msgnum = 50002, @severity = 10, 
            @msgtext = N'proc_LongRunningTrans has detected a batch running longer than %d minutes', 
            @lang = 'us_english', @with_log ='true'
go
...
if exists (select name from msdb.dbo.sysalerts where name = N'Blocking Alert')
   execute msdb.dbo.sp_delete_alert @name = N'Blocking Alert'
BEGIN
exec msdb.dbo.sp_add_alert @name = N'Blocking Alert'
, @message_id = 50001
, @severity = 0
, @enabled = 1
, @delay_between_responses = 1800
, @include_event_description_in = 4
, @notification_message = N'Please contact Support_dba to verify
Support_dba: Run zsqSupport_dbadb.dbo.proc_blocked to view blockers and terminate them if warranted.'
, @category_name = N'[Uncategorized]', @raise_snmp_trap = 0
END
...
go
-- may be still useful for login transfer, sorts out the sid issue
    ...
    begin
      ...
      begin -- SQL Server authentication
        ...
        set @tmpstr = 'CREATE LOGIN ' + quotename( @name ) +
					  ...
					  -- best part is the hashed password transfer
...
if exists (select *
             from sysobjects
            where id = object_id('dbo.sp_blocker_pss08') and
                  sysstat & 0xf = 4)
   drop procedure dbo.sp_blocker_pss08
go

create procedure dbo.sp_blocker_pss08
(
  @latch   int     = 1,
  @fast    int     = 1,
...
GO    -----------------------------------------------------------
----------------------------------- Create newlycreated utility db
------------------------------------------------------------
...
--  set @instance = '_'+ substring(@instance, charindex('\', @instance) + 1, datalength(@instance))
--else 
--  set @instance = ''

--if not exists(select *
--                from master.dbo.sysdatabases
--               where name = 'newlycreated utility db')
--begin
...
--  set @DataPath = (select Path
--                     from (select top 1 Path = substring(rtrim(F.filename),
--                                                         1,
--                                                         datalength(rtrim(F.filename)) / 2 -
--                                                                    charindex('\', reverse(rtrim(F.filename))))
--                             from master.dbo.sysaltfiles  F,
--                                  master.dbo.sysdatabases D
--                            where D.dbid = F.dbid and
--                                  F.status & 0x40 <> 0x40 and -- data file
--                                  F.filename like '%f:\mssql\data\%') as TEMP1)
...
--  set @LogPath = (select Path
--                    from (select top 1 Path = substring(rtrim(F.filename),
--                                                        1,
--                                                        datalength(rtrim(F.filename))/2 - charindex('\', reverse(rtrim(F.filename))))
--                            from master.dbo.sysaltfiles  F,
--                                 master.dbo.sysdatabases D
--                           where D.dbid = F.dbid and
--                                 F.status & 0x40 = 0x40 and -- Log file
--                                 F.filename like '%data%') as TEMP1)
...
--             @value=@LogPath output 

--  if @instance is not NULL or
--     @instance <> ''
--  begin
--    if @DataPath not like '%'++@instance+'%'
--      set @DataPath = @DataPath + @instance

--    if @LogPath not like '%'++@instance+'%'
--      set @LogPath = @LogPath + @instance
--  end

  -- make sure the path has a closing '\' - add one and remove if doubled
--  set @DataPath   = replace(@DataPath+ '\', '\\', '\')
--  set @LogPath    = replace(@LogPath + '\', '\\', '\')
--  set @sqlcommand = 'CREATE DATABASE [newlycreated utility db]  -- add actual db name before each _filename
--       ON PRIMARY (NAME = N'_Data'', FILENAME = '''+@DataPath + '_Data.mdf'', SIZE = 50, FILEGROWTH = 50) 
--   LOG ON (NAME = N''_Log'', FILENAME = '''+@LogPath + '_Log.ldf'' , SIZE = 50, FILEGROWTH = 50)'
...
--alter database [newlycreated utility db] set recovery simple
--go

--use newlycreated utility db
--go
...
------------------------------------------------ Create sp's
------------------------------------------------------------
--- restore alithya utility

------------------------------------------------------------
---------------------------------------------- Add sysadmins or other fixed roles?
------------------------------------------------------------

use [master]
go
--create login [gcosi\USERorgroup]    from windows with default_database=[master]
--- apply monitoring rights to all the service accounts from lines 917-920 above
-- add any others to sysadmin by default? or other roles? groups?
--exec master..sp_addsrvrolemember @loginame = N'gcosi\'       , @rolename = N'sysadmin'
-- or remove some and force the application of flexible server roles with some grants revoked (non dbas dont' need to restart system or make sys config changes)

if case when charindex('Enterprise Edition', @@version) > 0 or
             charindex('Standard Edition'  , @@version) > 0
        then 1
        else 0
   end = 0
begin
  create login [gcosi\support_dba] from windows with default_database=[master]
  exec master..sp_addsrvrolemember @loginame = N'gcosi\support_dba'    , @rolename = N'sysadmin'
end
go
--  perform post install due diligence, backup the system DBs to default location
BACKUP DATABASE [master] TO  DISK = N'F:\MSSQL\Backups\master.bak' 
WITH NOFORMAT, NOINIT,  NAME = N'master-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
GO
BACKUP DATABASE [model] TO  DISK = N'F:\MSSQL\Backups\model.bak'
 WITH NOFORMAT, NOINIT,  NAME = N'master-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
GO
BACKUP DATABASE [msdb] TO  DISK = N'F:\MSSQL\Backups\msdb.bak' 
WITH NOFORMAT, NOINIT,  NAME = N'master-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
GO

USE alithyaDBAUtility
go
--  But let's do the whole maintenance setup for next time, and the rest of the dba team, validate the dbs, the backup location,
-- and if the jobs aren't wanted, delete or disable them
/*
...
--USE alithyaDBAUtility -- Specify the database in which the objects will be created.
...
SET @BackupDirectory     = N'C:\Program Files\Microsoft SQL Server\MSSQL14.SQL2017\MSSQL\Backup\' -- Specify the backup root directory.
...
BEGIN
  ...
  ----------------------------------------------------------------------------------------------------
  --// Source: https://ola.hallengren.com adapted to Often the                                                          //--
  ----------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  SET @StartMessage = @StartMessage + 'Source: https://ola.hallengren.com adapted to Alithya' + CHAR(13) + CHAR(10)
  SET @StartMessage = REPLACE(@StartMessage,'%','%%') + ' '
  RAISERROR(@StartMessage,10,1) WITH NOWAIT
...
BEGIN
  ...
  --// Source: https://ola.hallengren.com adapted to Often the                                                          //--
  ----------------------------------------------------------------------------------------------------
-- Hugo Shebbeare, July 2018, adjusted job names to Alithya
-- NOTHING IS SCHEDULED ON PURPOSE - adjust them manually afterwards.
  SET NOCOUNT ON
...
BEGIN
  ...
  SET @JobDescription = 'Source: https://ola.hallengren.com adapted to Alithya'
  SET @JobCategory = 'Database Maintenance'
  SET @JobOwner = SUSER_SNAME(0x01)

  SET @JobName01 = 'DBA_Backup-SYSTEM_DATABASES-FULL'
  ...
  SET @JobName02 = 'DBA_Backup-USER_DATABASES-DIFF'
  ...
  SET @JobName03 = 'DBA_Backup-USER_DATABASES-FULL'
  ...
  SET @JobName04 = 'DBA_Backup-USER_DATABASES-LOG'
  ...
  SET @JobName05 = 'DBA_IntegrityCheck-SYSTEM_DATABASES'
  ...
  SET @JobName06 = 'DBA_IntegrityCheck-USER_DATABASES'
  ...
  SET @JobName07 = 'DBA_IndexOptimize-USER_DATABASES'  -- HS and Calin observed that this had be tweaked for us, example in GE, tell it to do stats and online rebuild only
  SET @JobCommand07 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[IndexOptimize] @UpdateStatistics = ''ALL'', @TimeLimit=''8300'', @OnlyModifiedStatistics = ''Y'', @Databases = ''USER_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
...
END
GO
--- end OLA's adjusted maintenance jobs
